  ...
  try {
    ...
    if ((!result || !result.token || !result.user) && hasDb() && sql) {
      const dbUsers = await sql`
        SELECT id, email, display_name, is_admin, points, password_hash
        FROM users
    ...
    if ((!result || !result.token || !result.user) && hasDb() && sql) {
      ...
      if (fallbackEmail && fallbackPassword && email.toLowerCase() === fallbackEmail && password === fallbackPassword) {
        let rows = await sql`SELECT id, email, display_name, is_admin, points FROM users WHERE LOWER(email) = ${fallbackEmail} LIMIT 1`;
        let userRow = rows[0];
        if (!userRow) {
